...
public:                                                                                    \
    Q_PROPERTY(_type_ _prop_ READ _prop_ WRITE set_##_prop_ NOTIFY changed_##_prop_ FINAL) \
    Q_SIGNAL void changed_##_prop_();                                                      \
    _type_        _prop_() const { return prop<_type_>(this->api().input._input_); }       \
    void          set_##_prop_(_type_ v) {                                                 \
        set_prop<_type_>(v, this->api().input._input_, [this]() {                 \
            changed_##_prop_();                                                   \
        });                                                                       \
    }
