...
class ObjectBindableProperty : public QPropertyData<T> {
    ...
    static bool constexpr HasSignal = ! rstd::mtp::same_as<decltype(Signal), rstd::nullptr_t>;
    using SignalTakesValue          = rstd::mtp::is_invocable<decltype(Signal), Class, T>;
...
public:
    ...
    explicit ObjectBindableProperty(const T& initialValue, Class* p)
        : QPropertyData<T>(initialValue), m_owner(p) {}
    explicit ObjectBindableProperty(T&& initialValue, Class* p)
        : QPropertyData<T>(rstd::move(initialValue)), m_owner(p) {}
    ...
    QPropertyBinding<T> setBinding(
        Functor&&                             f,
        const QPropertyBindingSourceLocation& location = QT_PROPERTY_DEFAULT_BINDING_LOCATION,
        rstd::mtp::enable_if_t<rstd::mtp::is_invocable_v<Functor>>* = nullptr) {
    ...
    QPropertyChangeHandler<Functor> onValueChanged(Functor f) {
        static_assert(rstd::mtp::is_invocable_v<Functor>,
