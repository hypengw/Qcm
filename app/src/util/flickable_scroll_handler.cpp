...
FlickableScrollHandler::FlickableScrollHandler(QObject* parent): QObject(parent) {
    connect(this, &FlickableScrollHandler::scaleFactorChanged, this, [this]() {
        m_effectiveScaleFactor =
            QGuiApplication::styleHints()->wheelScrollLines() * 20 * m_scaleFactor;
...
FlickableScrollHandler::~FlickableScrollHandler() { detach(); }
...
void FlickableScrollHandler::init() {
    ...
    m_propertyInteractive   = QQmlProperty(m_target, "interactive", qCtx);
    ...
    m_propertyInteractive.connectNotifySignal(this, SLOT(refreshAttach()));
    connect(this,
            &FlickableScrollHandler::enabledChanged,
            this,
            &FlickableScrollHandler::refreshAttach);

    emit enabledChanged();
    emit initialized();
}
void FlickableScrollHandler::refreshAttach() {
    if (enabled() && m_propertyInteractive.read().toBool())
        attach();
    else
        detach();
}
...
bool FlickableScrollHandler::eventFilter(QObject* watched, QEvent* event) {
    ...
    const auto handler = [this, wheel, ev](Qt::Orientation orientation, bool fallback = false) {
        ...
        const auto handler = [this, vertical](qreal delta, Type type) {
            ...
            const auto pos =
                vertical ? m_propertyContentY.read().toReal() : m_propertyContentX.read().toReal();
            const auto size =
                vertical ? m_propertyHeight.read().toReal() : m_propertyWidth.read().toReal();

            if (contentSize < size || pos >= contentSize) return false;
        ...
        if (_delta != 0 && (wheel->modifiers() & (Qt::ControlModifier | Qt::ShiftModifier))) {
            _type = Type::Pixel;
            _delta =
                (_vertical ? m_propertyHeight.read().toReal() : m_propertyWidth.read().toReal()) *
                (_delta > 0 ? 1 : -1);
        }
#endif
...
qreal FlickableScrollHandler::scaleFactor() const { return m_scaleFactor; }

void FlickableScrollHandler::setScaleFactor(qreal newScaleFactor) {
    if (qFuzzyCompare(m_scaleFactor, newScaleFactor)) return;
    m_scaleFactor = newScaleFactor;
    emit scaleFactorChanged();
}

bool FlickableScrollHandler::enabled() const { return m_enabled; }
...
void FlickableScrollHandler::adjustScrollBarV() { adjustScrollBar(m_scrollBarV); }

void FlickableScrollHandler::adjustScrollBarH() { adjustScrollBar(m_scrollBarH); }

qreal FlickableScrollHandler::effectiveScaleFactor() const { return m_effectiveScaleFactor; }
